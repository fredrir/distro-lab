...
in
{
  # The persistent disk is the home directory itself, so a checkout sits at
  # ~/<repo> rather than one level down.  Everything the root image carries is
  # disposable; everything below here survives `just rebuild`.
  fileSystems.${homeDir} = {
    device = "/dev/vdb";
    fsType = "ext4";
  ...
  systemd.services.dlab-home-perms = {
